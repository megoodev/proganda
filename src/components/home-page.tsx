      ...
      <section className="border-t border-white/10 bg-[#151515] px-5 py-20 lg:px-8 lg:py-24">
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
          <div>
            ...
            <Link href="/services" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-[#ccff00]">
              {t("integrationsCta")} <ArrowRight className="size-4" />
            </Link>
          </div>
          <div className="relative grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="pointer-events-none absolute left-1/2 top-1/2 hidden h-px w-[70%] -translate-x-1/2 bg-[#ccff00]/30 sm:block" />
            <div className="pointer-events-none absolute left-1/2 top-1/2 hidden h-[70%] w-px -translate-y-1/2 bg-[#ff007f]/30 sm:block" />
            <div className="col-span-2 row-span-2 flex min-h-44 items-center justify-center border border-[#ccff00]/50 bg-[#0d0d0d] p-6 sm:col-span-2 sm:row-span-2">
              <div className="text-center">
                <div className="mx-auto flex size-20 items-center justify-center overflow-hidden bg-white p-2">
                  <img src="/assits/logos/IMG_1531.PNG" alt="ProGanda" className="size-full object-contain" />
                </div>
                <p className="mt-4 text-xs font-black uppercase tracking-[0.2em] text-[#ccff00]">PROGANDA / HUB</p>
              </div>
            </div>
            {["IMG_0092.JPG.jpeg", "IMG_9506.JPG.jpeg", "IMG_9508.PNG"].map((logo, index) => (
              <div key={logo} className="relative z-10 flex min-h-20 items-center justify-center border border-white/10 bg-[#1d1d1d] p-4 transition hover:border-[#ccff00] hover:bg-[#222222]">
                <img src={`/assits/logos/${logo}`} alt={`Integration ${index + 1}`} className="max-h-14 w-full object-contain mix-blend-screen" />
              </div>
            ))}
            <div className="relative z-10 flex min-h-20 items-center justify-center border border-[#ff007f]/40 bg-[#ff007f]/10 p-4 text-center text-[10px] font-black uppercase tracking-[0.16em] text-[#ff007f]">
