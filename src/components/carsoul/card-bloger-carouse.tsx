    ...
    cardElements.forEach((card, cardIndex) => {
      ...
      if (slot !== undefined) {
        ...
        if (isFirstMount) {
          ...
          const enterX = (direction === "right" ? 40 : -40) * movementSign;
          gsap.set(card, {
            x: `${enterX}rem`,
            y: `${y * hMult}rem`,
            rotation:
              direction === "right" ? 30 * movementSign : -30 * movementSign,
            scale: 0.5,
            opacity: 0,
          });
          gsap.to(card, {
        ...
        const exitX = (direction === "right" ? -40 : 40) * movementSign;
        gsap.to(card, {
          x: `${exitX}rem`,
          opacity: 0,
          scale: 0.5,
          rotation: (direction === "right" ? -30 : 30) * movementSign,
          duration: 0.4,
          ease: "power2.in",
          zIndex: 0,
        });
        ...
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span
                key={i}
                className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-black/90 dark:bg-white/90 scale-[1.3]" : "bg-black/35 dark:bg-white/25"}`}
              />
            ))}
          </div>
          <button
